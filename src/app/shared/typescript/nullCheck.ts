...
export function isNotNull<T>(arg: T | null): arg is T {
    return arg !== null; // to exclude only null
}
export function isNotNullOrUndefined<T>(arg: T | null | undefined): arg is T {
    return arg != null; // to exclude only null and undefined
}
export function isTruthy<T>(arg: T | null | undefined): arg is T {
    return !!arg; //to exclude all falsy values (like null, undefined, 0, NaN, an empty string "", etc.)
}